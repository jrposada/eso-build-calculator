...
Add missing skills to a skill line data file based on raw datamined skill data.

## Arguments

- `$ARGUMENTS` - (Optional) Path in format `<class>/<skill-line>` (e.g., `nightblade/shadow`)

## Instructions

1. If `$ARGUMENTS` is empty or not provided:
   - List all `.txt` files in `datamine/skills/` recursively
   - Process each file following steps 2-9 below
   - The path for each file is derived from its location (e.g., `datamine/skills/nightblade/shadow.txt` → `nightblade/shadow`)

2. Read the raw skill data from `datamine/skills/$ARGUMENTS.txt` (e.g., `datamine/skills/nightblade/shadow.txt`).

3. Read the target skill file at `src/data/skills/$ARGUMENTS.ts` to understand:
   - The current skills already defined
   - The ESO class and skill line name being used
   - The existing code structure and formatting

4. Read `src/models/skill.ts` to understand the Skill interface and available types.

5. Cross-reference the raw data with the existing skills to identify missing skills.

6. For each skill line in the raw data, identify:
...
7. When adding skills, follow these conventions:
   ...
   **IMPORTANT**: Healing over time (HoT) is NOT damage over time. If a skill deals instant damage and then heals over time, only record the instant damage in `hits`. Do not use `dot` fields for healing effects. Example: "dealing 1548 Magic Damage and healing you for 50% every 2 seconds" = `hits: [{ value: 1548 }]` (the healing is ignored).

8. Organize skills by their skill line (base + morphs grouped together) with comments.

9. After adding skills, run `npx tsc --noEmit` to verify there are no TypeScript errors related to your changes.
